...
{
    /**
     * Returns a list of the names of all zyre variables
     *
     * @param data_logger a DataLogger shared pointer
     */
    std::vector<std::string> ZyreQueryInterface::getVariables(const std::shared_ptr<loggers::DataLogger> data_logger)
    {
        std::vector<std::string> variables = data_logger->getVariables(config::DataSourceNames::ZYRE);
        return variables;
    }

    /**
     * Returns a data dictionary in which each key represents a zyre variable
     ...
     */
    std::map<std::string, std::vector<std::string>> ZyreQueryInterface::getData(const std::shared_ptr<loggers::DataLogger> data_logger,
