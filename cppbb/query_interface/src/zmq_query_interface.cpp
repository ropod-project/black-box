...
{
    /**
     * Returns a list of the names of all ZMQ variables
     *
     * @param data_logger a DataLogger shared pointer
     */
    std::vector<std::string> ZmqQueryInterface::getVariables(const std::shared_ptr<loggers::DataLogger> data_logger)
    {
        std::vector<std::string> variables = data_logger->getVariables(config::DataSourceNames::ZMQ);
        return variables;
    }

    /**
     * Returns a data dictionary in which each key represents a ZMQ variable
     ...
     */
    std::map<std::string, std::vector<std::string>> ZmqQueryInterface::getData(const std::shared_ptr<loggers::DataLogger> data_logger,
